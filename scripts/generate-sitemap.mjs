#!/usr/bin/env node
/**
 * Regenerate public/sitemap.xml from current hotels in Supabase.
...
const areas = ["ayia-napa", "limassol", "paphos"];
const cats = ["luxury", "family", "budget"];

const res = await fetch(
  `${SUPABASE_URL}/rest/v1/hotels?select=area,category,hotel_slug&order=area`,
  { headers: { apikey: ANON } },
);
...
const add = (loc, prio = "0.5") =>
  urls.push(`  <url><loc>${SITE}${loc}</loc><lastmod>${today}</lastmod><priority>${prio}</priority></url>`);
...
for (const h of hotels) add(`/hotell/${h.area}/${h.category}/${h.hotel_slug}`, "0.7");
